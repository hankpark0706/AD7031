...
%
% Probability (old Sections 7-8) now lives in OUU_week04_note.tex.
%
...
% REMOVED: Section 7 (probability review) covered OUU Lecture 3, pp. 1-2 (top),
...
% REMOVED: Section 8 was the running example -- Markowitz, with the session's data
...
{\Large\bfseries Week 2 --- Convexity and Linear Programming}\\[2pt]
...
keep an eye on that $\xi$ --- \emph{uncertain data} is exactly what this course is about. it returns in the week 4 note.
\end{board}
...
\quad
\textbullet\ Markowitz (week 4 note, Section 10): $\min\, x^{\top}\Sigma x$ \ s.t.\ $\mu^{\top}x \ge r$, \ $\mathbf{1}^{\top}x = 1$, \ $x \ge 0$
\vspace{6pt}
...
\end{tabular}
\end{board}
\takeaway{convexity is compositional: certify the pieces, then check the operation. the risk measures and worst-case objectives ahead are built exactly this way.}

% ---------------------------------------------------------------- next
\par\medskip
{\color{ruleink}\rule{\textwidth}{1.2pt}}\par\nobreak\vspace{3pt}
{\color{keyink}\small\textbf{next}\quad the probability review, and the Markowitz portfolio --- see the week 4 note.}
