...
\quad two coins. \quad $A = \{\text{exactly one head}\} = \{HT, TH\}$, \qquad
      $B = \{\text{first flip is head}\} = \{HH, HT\}$
\vspace{3pt}

\quad $\mathbb{P}(A \cap B) = \mathbb{P}(\{HT\}) = \tfrac14$, \qquad
      $\mathbb{P}(A)\,\mathbb{P}(B) = \tfrac12 \cdot \tfrac12 = \tfrac14$
\qquad equal --- \textbf{independent}
\vspace{3pt}

\quad so $\mathbb{P}(A \mid B) = \tfrac12 = \mathbb{P}(A)$: \ the first flip says nothing about whether there is exactly one head
\vspace{8pt}

\quad now \ $A = \{\text{both heads}\} = \{HH\}$, \qquad
      $B = \{\text{first flip is head}\} = \{HH, HT\}$ \quad --- the same test:
\vspace{3pt}

\quad $\mathbb{P}(A \cap B) = \mathbb{P}(\{HH\}) = \tfrac14$, \qquad
      $\mathbb{P}(A)\,\mathbb{P}(B) = \tfrac14 \cdot \tfrac12 = \tfrac18$
\qquad $\tfrac14 \ne \tfrac18$ --- \textbf{dependent}
\vspace{3pt}

\quad which is why conditioning moved it: \ $\mathbb{P}(A \mid B) = \tfrac12 \ne \tfrac14 = \mathbb{P}(A)$
\vspace{6pt}

\quad {\color{punchink}$B$ did not change, $A$ did --- and the verdict flipped.} \ independence is a property of the
      \emph{pair}, not of either event on its own.
\vspace{10pt}
...
\quad two coins again: \ $A_1 = \{\text{first flip is head}\}$, \ $A_2 = \{\text{first flip is tail}\}$ --- a partition, each $\tfrac12$
\vspace{4pt}
...
$\mathbb{P}(\mu - 2\sigma \le \tilde\xi \le \mu + 2\sigma) \approx 0.95$, \qquad
$\mu \pm 3\sigma \approx 0.997$
\vspace{10pt}

\textbf{Six Sigma} --- yes, the manufacturing one \quad push the tolerance limits out to $\mu \pm 6\sigma$:
\vspace{4pt}

\quad $\mathbb{P}(\mu - 6\sigma \le \tilde\xi \le \mu + 6\sigma) = 0.999999998$
\qquad --- about $2$ failures per \emph{billion} parts
\vspace{6pt}

\quad so why is the slogan \textbf{3.4 defects per million}, a million times worse? \ because the standard
      also allows the process mean to \textbf{drift} by up to $1.5\sigma$ over a long production run.
\vspace{3pt}

\quad the mean slides, the limits do not --- the nearer one is then only $4.5\sigma$ away, and only that tail matters:
\vspace{3pt}

\quad $\mathbb{P}(\tilde\xi \text{ past the nearer limit}) = 3.4 \times 10^{-6}$
\qquad --- the famous $3.4$ per million
\vspace{6pt}

\quad {\color{punchink}notice the shape of the requirement:} \ \emph{choose the design so that failure has
      probability at most $\varepsilon$}. \ that is a \textbf{chance constraint}, and it is one of the
      paradigms this course builds --- the same $\tilde\xi$, collapsed to a number a different way.
\vspace{4pt}
