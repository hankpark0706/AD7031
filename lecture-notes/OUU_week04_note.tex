...
\quad {\color{punchink}the scenario index goes \emph{up}:} \ $\xi^s$ is scenario $s$; \ a \emph{subscript} $\xi_j$ is
      component $j$ of a vector (Section 4). two different jobs, two different slots.
\vspace{6pt}
...
\end{board}
%% ------------------------------------------------------------------ 10
\beat{4}{Running example: the Markowitz portfolio}
\begin{board}
...
\textbf{Random vectors} \quad the data is a vector now, so the mean is one too and the spread is a matrix:
\vspace{4pt}

\quad mean vector \ $\mu \in \mathbb{R}^n$, \ $\mu_j = \mathbb{E}[\tilde\xi_j]$
\vspace{3pt}

\quad $\mathrm{Cov}(\tilde\xi_i, \tilde\xi_j) = \mathbb{E}\big[(\tilde\xi_i - \mu_i)(\tilde\xi_j - \mu_j)\big]$
\qquad --- how the two move \emph{together}; $\mathrm{Cov}(\tilde\xi_j, \tilde\xi_j) = \mathrm{Var}(\tilde\xi_j)$
\vspace{3pt}

\quad covariance matrix \ $\Sigma \in \mathbb{R}^{n \times n}$, \ $\Sigma_{ij} = \mathrm{Cov}(\tilde\xi_i, \tilde\xi_j)$:
\ diagonal = the variances, \ symmetric, \ always $\Sigma \succeq 0$
\vspace{6pt}

\quad {\color{punchink}the one fact the model runs on} --- for a fixed $c \in \mathbb{R}^n$:
\vspace{3pt}

\qquad $\mathbb{E}\big[c^{\top}\tilde\xi\big] = c^{\top}\mu$, \qquad
       $\mathrm{Var}\big(c^{\top}\tilde\xi\big) = c^{\top}\Sigma c$
\vspace{3pt}

\quad --- the first is linearity (Section 2); the second is linearity used twice, on a product of two terms
\vspace{10pt}
...
\quad the fact above with $c = x$: \qquad
$\mathbb{E}[\tilde\xi^{\top}x] = \mu^{\top}x$, \qquad $\mathrm{Var}(\tilde\xi^{\top}x) = x^{\top}\Sigma x$
\vspace{10pt}

\textbf{Check --- two assets} \quad the joint distribution below, \ half the budget in each: \ $x = (\tfrac12, \tfrac12)$
\vspace{4pt}
...
\quad the loss $-\tilde\xi^{\top}x$ is random; Markowitz collapses it with two numbers ---
\vspace{1pt}
