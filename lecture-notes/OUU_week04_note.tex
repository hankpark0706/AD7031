...
\quad now run the same test on the \emph{first} pair, \ $A = \{\text{both heads}\} = \{HH\}$:
\vspace{3pt}
...
\quad which is why conditioning moved it there: \ $\mathbb{P}(A \mid B) = \tfrac12 \ne \tfrac14 = \mathbb{P}(A)$
\vspace{6pt}

\quad {\color{punchink}same $B$, two different $A$, opposite answers} --- independence is a property of the \emph{pair},
      not of either event on its own.
\vspace{10pt}
